...
def _open_text_file(path):
  """Open a text file as seekable Unicode stream for Python 3."""
...
def testeqsupport():
  ...
  if rc != 0:
    ...
    msg += '  latex: ' + p.stdout.readlines()[0].decode(errors='replace').rstrip() + '.\n'
  ...
  if rc != 0:
    ...
    msg += '  dvipng: ' + p.stdout.readlines()[0].decode(errors='replace').rstrip() + '.\n'
...
class controlstruct(object):
  ...
  def pushfile(self, newfile):
    ...
    self.inf = _open_text_file(newfile)
...
def standardconf():
  ...
  [lastupdated]
  Copyright 2026 Jichu Li. Page generated |.
...
def parseconf(cns):
  ...
  for sname in cns:
    fs.append(_open_text_file(sname))
  ...
  try:
    ...
    if rc != 0:
      for l in p.stdout.readlines():
        print('  ' + l.decode(errors='replace').rstrip())
      exts.remove('.tex')
    ...
    rc = p.wait()
    if rc != 0:
      print([x.decode(errors='replace') for x in p.stderr.readlines()])
      raise Exception('dvipng error')
    depth = int(p.stdout.readlines()[-1].decode(errors='replace').split('=')[-1])
  finally:
    # Clean up.
    for ext in exts:
      g = basefile + ext
